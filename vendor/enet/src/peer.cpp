...
enet_peer_send (ENetPeer * peer, enet_uint8 channelID, ENetPacket * packet)
{
   ...
   if (packet -> dataLength > fragmentLength)
   {
             ...
             fragmentOffset += fragmentLength)
      {
		 ...
		 fragment = new(std::nothrow) ENetOutgoingCommand(); //fx::generic_object_pool<ENetOutgoingCommand>::allocate();
         if (fragment == NULL)
         {
...
enet_peer_reset_queues (ENetPeer * peer)
{
	...
	while (!enet_list_empty(&peer->acknowledgements))
		delete (ENetAcknowledgement*)enet_list_remove(enet_list_begin(&peer->acknowledgements));
...
enet_peer_queue_acknowledgement (ENetPeer * peer, const ENetProtocol * command, enet_uint16 sentTime)
{
	...
	acknowledgement = new(std::nothrow) ENetAcknowledgement(); //(ENetAcknowledgement*)fx::generic_object_pool<ENetAcknowledgement>::allocate();
...
{
	ENetOutgoingCommand* outgoingCommand = new(std::nothrow) ENetOutgoingCommand(); // fx::generic_object_pool<ENetOutgoingCommand>::allocate();
...
enet_peer_queue_incoming_command (ENetPeer * peer, const ENetProtocol * command, const void * data, size_t dataLength, enet_uint32 flags, enet_uint32 fragmentCount)
{
	...
	incomingCommand = new(std::nothrow) ENetIncomingCommand(); // fx::generic_object_pool<ENetIncomingCommand>::allocate();
